 ...
 */
export async function executeLedgerCredit({ nodeId, amountUsd, type, referenceNodeId = null, idempotencyKey = null, description = '' }) {
  ...
  try {
    ...
    await query(
      `INSERT INTO wallet_ledger (node_id, amount_usd, amount_inr, transaction_type, reference_node_id, idempotency_key, description)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [nodeId, finalCreditUsd, finalCreditInr, type, referenceNodeId, idempotencyKey, description]
    );
    ...
    await query('ROLLBACK');
    if (error.code === '23505') {
      console.warn('[MLM Ledger] Duplicate idempotency key or daily yield detected. Skipping.');
      return { success: false, message: 'Duplicate transaction (Idempotency check failed)' };
    }
